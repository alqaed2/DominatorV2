...
def _keywords(niche: str, limit: int = 8) -> List[str]:
    ...
    # split arabic/latin words and numbers
...
def _ensure_niche_lock(niche: str, payload: Dict[str, Any]) -> None:
    """
    Hard guard: if niche doesn't appear in outputs, we treat it as invalid.
    """
    niche = _clean_text(niche)
    if not niche:
    ...
    if niche not in blob:
        # If the exact niche phrase didn't appear, still accept if keywords appear
...
def _build_visual_prompt(niche: str, lang: str) -> str:
    niche = _clean_text(niche)
    # Prompt عام لكن يربط النيش بسياق بصري واضح
    return (
...
def process_build_pack(job_id: str) -> Dict[str, Any]:
    ...
    - mark processing
    - generate pack payload with strict niche-lock
    - insert pack row
    - update job row to done + store result
    """
    jobs, packs = _reflect_tables()
    engine = _get_engine()

    job_id_key = _try_uuid(job_id)
    ...
    # possible request/payload columns
    ...
    with engine.begin() as conn:
        ...
        # generate payload (strict niche lock)
        ...
        pack_id_value = pack_uuid if str(packs.c[packs_id_col].type).lower().find("uuid") >= 0 else pack_uuid.hex

        pack_row: Dict[str, Any] = {packs_id_col: pack_id_value}
        if packs_job_id_col and jobs_id_col:
            pack_row[packs_job_id_col] = job_id_key

        # try to store common fields if columns exist
        for cname, val in [
        ...
        # update job as done + attach pack_id + result
    ...
    # return minimal payload for logs/debug
    return {"ok": True, "job_id": job_id, "pack_id": str(pack_id_value), "niche": niche, "ts": _utc_now_iso()}


def worker_tick(limit: int = 1) -> Dict[str, Any]:
    """
    Process up to N queued jobs quickly.
